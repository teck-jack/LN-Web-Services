...
import { Bell, Menu, User, Settings, LogOut, Shield, MessageSquare, Check } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-card shadow-sm">
      <div className="flex h-16 items-center px-4 md:px-6">
        ...
        <Link to="/" className="flex items-center space-x-2">
          ...
          <span className="font-bold text-lg hidden sm:inline-block">LN Services</span>
        </Link>
                    ...
                    notifications.map((notification) => (
                      ...
                      >
                        <div className="flex w-full items-start justify-between gap-2">
                          <span className={cn("text-sm font-medium leading-none", !notification.isRead && "text-primary")}>
                            {notification.title}
                          </span>
                          {!notification.isRead && (
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-4 w-4 hover:bg-transparent text-muted-foreground hover:text-primary"
