...
import { Search, Filter, Download, IndianRupee, CreditCard, Calendar, TrendingUp, FileText } from "lucide-react";
...
export default function PaymentHistory() {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Button
                    ...
                    onClick={() => navigate(`/admin/cases/${row.caseId?._id}`)}
                >
                    {row.caseId?.caseId}
                </Button>
            ),
        ...
        {
            header: "Enrolled By",
            accessor: (row: any) => row.paymentMetadata?.enrolledBy?.name || 'N/A',
            cell: (row: any) => (
                <div>
                    <p className="font-medium text-sm">{row.paymentMetadata?.enrolledBy?.name || 'N/A'}</p>
                    <Badge variant="outline" className="text-xs capitalize">
                        {row.paymentMetadata?.enrollerRole || 'N/A'}
                    </Badge>
                </div>
            ),
        },
            ...
            {analytics && (
                <div className="grid gap-4 md:grid-cols-4">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                                From {analytics.totalPayments} transactions
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">₹{Math.round(analytics.averageTransaction).toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground mt-1">
                                Per transaction
                            </p>
                        </CardContent>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">₹{analytics.totalDiscount.toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground mt-1">
                                Savings provided
                            </p>
                        </CardContent>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Select value={paymentMethodFilter} onValueChange={setPaymentMethodFilter}>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="test_payment">Test Payment</SelectItem>
                    </SelectContent>
                </Select>
            </div>
