...
export default function ServiceDetails() {
  ...
  const fetchService = async (serviceId: string) => {
    try {
      setLoading(true);
      const response = await associateService.getServiceById(id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" onClick={() => navigate("/associate/services")}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          <Button
            className="w-full"
            size="lg"
            onClick={() => navigate(`/associate/users/create?serviceId=${service._id}`)}
          >
