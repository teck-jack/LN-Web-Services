...
import { Search, Filter, Download, IndianRupee, Users, Percent, TrendingUp, FileText } from "lucide-react";
...
export default function PaymentHistory() {
    ...
    const calculateCommission = (amount: number) => {
        const rate = analytics?.commissionRate || 10;
    ...
    const columns = [
        ...
        {
            header: "Commission",
            accessor: (row: any) => calculateCommission(row.amount),
            cell: (row: any) => (
                <div className="text-sm">
                    <span className="font-semibold text-success">
                        ₹{calculateCommission(row.amount).toLocaleString()}
                    </span>
                    <p className="text-xs text-muted-foreground">
                        {analytics?.commissionRate || 10}% rate
                    </p>
                </div>
            ),
        },
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Payment History & Commissions</h1>
                    <p className="text-muted-foreground mt-2">Track payments from your onboarded users</p>
                </div>
            ...
            {analytics && (
                <div className="grid gap-4 md:grid-cols-4">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-success">
                                ₹{analytics.estimatedCommission?.toLocaleString() || 0}
                            </div>
                            <p className="text-xs text-muted-foreground mt-1">
                                At {analytics.commissionRate || 10}% rate
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analytics.onboardedUsersCount || 0}</div>
                            <p className="text-xs text-muted-foreground mt-1">
                                Total users
                            </p>
                        </CardContent>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analytics.commissionRate || 10}%</div>
                            <p className="text-xs text-muted-foreground mt-1">
                                Your rate
                            </p>
                        </CardContent>
                    ...
                    {payments.length === 0 && (
                        <div className="text-center py-12">
                            ...
                            <p className="text-sm text-muted-foreground mt-2">
                                Payments from your onboarded users will appear here
                            </p>
                        </div>
                    )}
                </>
